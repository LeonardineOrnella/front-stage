...
export default function SideBar() {
  ...
  const links = [
    ...
    { href: "/dasboard/formation", label: "Mes Formations", icon: GraduationCap, category: "main" },
    ...(user?.role === "admin"
      ? [
  ...
  return (
    <div className="flex min-h-screen">
      {/* Sidebar */}
      <aside className="w-72 bg-emerald-600 text-white flex flex-col shadow-2xl border-r border-emerald-700/30">
        {/* Header avec logo */}
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          {user?.role === "admin" && (
            <div className="mb-6">
              ...
              {links.filter(link => link.category === "admin").map(({ href, label, icon: Icon }) => (
                <Link
                  key={href}
                  href={href}
                  className={`group flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 relative overflow-hidden ${
